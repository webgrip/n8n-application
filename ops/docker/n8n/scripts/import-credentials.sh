...
if [ -f "/data/credentials/openapi.json" ]; then
    ...
    jq --arg apiKey "$OPENAI_API_KEY" '.data.apiKey = $apiKey' /data/credentials/openapi.json > tmp && mv tmp /data/credentials/openapi.json
else
...
if [ -f "/data/credentials/telegram.json" ]; then
    ...
    jq --arg apiKey "$TELEGRAM_API_KEY" '.data.accessToken = $apiKey' /data/credentials/telegram.json > tmp && mv tmp /data/credentials/telegram.json
else
